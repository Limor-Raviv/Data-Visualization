...
#using a basic plot, in base R, check that our data is normally distributed (this is a pre-condition for running many statistical analyses)
...
# To make, first tabulate data. Here, the levels of nativeBorn, including NA. Use summary() for this, not table(), because NAs are excluded from that function.
...
## we can also add the raw data points instead of error bars: justd plot the summarized line on top on the full data!
...
# This will provide a confidence band using an underlying function. We asked for 'loess' which is a sort of rolling average.
...
For this, we'll use the tweenr and gg_animate packages.

```{r animated plot, include=FALSE, warning=FALSE, cache=TRUE}
...
d_sum_by_age$ease <- "linear" #add a coloumn to the data frame

d_animate<- tween_elements(d_sum_by_age, 'age', 'nativeBorn','ease', nframes = 100) # create 100 frames between each age
#The tween_elements functions creates two additional coloums to your data frame: ".frame" (corresponding to the origina trial number) and ".group" (corresponding to the original participant number).

# we save the same plot, but add the new variables

plot3 <- ggplot(data=d_animate,aes(x = age, y = mVocab, group=.group, color=.group, frame=.frame, cumulative=TRUE))+ 
  geom_line(lwd=2)+
  scale_x_continuous("Age")+
  scale_y_continuous("Vocabulary Score")+
  ggtitle("Vocabulary by age and nativeness")+
...
gganimate(plot3, interval = 0.05) # this may take a few seconds

# save as a GIF: 
gganimate(plot3, title_frame = FALSE, filename="animated_plot.gif",ani.height=500, ani.width=700, interval = 0.05) 
# remove the changing title by frame by adding title_frame = FALSE
```
...
model_plot$nativeBorn <- factor(model_plot$nativeBorn, levels=c("Non-native", "Native")) # make sure our variable is the same (you can check to see what happens if we don't do this...)
 ...
 plot3 <- ggplot()+  ## this plot pulls a lot of stuff together, so keep the specifications in the geoms
